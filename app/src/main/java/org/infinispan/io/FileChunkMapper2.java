...
/**
 * Takes care of properly storing and retrieving file chunks from/to cache. Each chunk's key is composed of the file
 * path and the chunk's number. The value is a byte array, which is either chunkSize bytes long or less than that in the
 * case of the last chunk.
 *
 * @author Marko Luksa
 */
class FileChunkMapper2
{

   private final GridFile file;
   private final Cache<String, byte[]> cache;

   public FileChunkMapper2(GridFile file, Cache<String, byte[]> cache)
   {
      this.file = file;
      this.cache = cache;
   }
